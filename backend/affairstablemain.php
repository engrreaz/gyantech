<table class="table table-hover table-bordered" id="sampleTable">
    ...
    <tbody>
        <?php
        if ($subcat != '') {
            $sql5 = "SELECT * FROM currentaffairs where title!='' and topic='$topic' and category = '$cat' and subcategory='$subcat'  order by id desc";
        } else if ($cat != '') {
            $sql5 = "SELECT * FROM currentaffairs where title!='' and topic='$topic' and category = '$cat'  order by id desc";
        } else if ($topic != '') {
            $sql5 = "SELECT * FROM currentaffairs where title!='' and topic='$topic'  order by id desc";
        } else {
            $sql5 = "SELECT * FROM currentaffairs where title!=''   order by id desc";
        }
        // echo $sql5;
        ...
        if ($result6x->num_rows > 0) {
            while ($row5 = $result6x->fetch_assoc()) {
                ...
                ?>
                <tr>
                    <td class="text-small"><small><?php echo $topic . '<br>' . $cat . ' <i class="fa fa-circle"></i> '  . $subcat; ?></small></td>
                    <td>
                        ...
                        <div id="detailsblock<?php echo $id; ?>" class="" style="display:none;">
                            <div class="d-flex">
                                <button class="btn btn-primary p-1 mr-1" onclick="status(<?php echo $id; ?>, 0);" <?php if ($stst == 0)
